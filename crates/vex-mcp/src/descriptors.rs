    ...
    let mut tools = serde_json::json!([
        {
            ...
            "inputSchema": {
                ...
                "properties": {
                    ...
                    "exclude_generated": { "type": "boolean", "description": "Drop machine-generated files (protobuf stubs, sqlc output, bindgen bindings, ORM schemas) from the results, recognised from the generator's header banner. Pass on repos that check in generated code, where the stubs outnumber and outrank hand-written symbols. Heuristic: a generator that writes no banner is not detected, so it under-reports rather than hiding hand-written code.", "default": false }
                },
                "required": ["query"]
            }
        },
