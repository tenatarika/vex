...
fn tool_descriptors() -> Value {
    serde_json::json!([
        ...
        {
            "name": "callers",
            "description": "Direct callers of a function via the persistent call-graph FST (~4ms when indexed; falls back to live-scan). Prefer over grep for `who calls Foo?` — grep on the function name hits doc comments and string literals; the call-graph edges are resolved at parse time. Phase 14.2 + 14.2.2: Python/Java function/method decorators and Kotlin annotations / C# method+constructor attributes emit forward edges, so `callers GetMapping` lists every Spring handler, `callers get` lists every FastAPI route, `callers HttpGet` every ASP.NET action, `callers JvmStatic` every Kotlin function annotated `@JvmStatic` (the rightmost identifier of `@app.get` / `[System.Web.Mvc.HttpGet]` / `@kotlin.jvm.JvmStatic` becomes the callee). Note the rightmost-identifier convention means `callers get` mixes decorator handlers with any regular `.get()` call — narrow with `include`/`exclude` if needed. Pair with `paths` for multi-hop chains.",
            "inputSchema": {
                "type": "object",
                "properties": {
        ...
        {
            "name": "callees",
            "description": "Direct callees of a function via the persistent call-graph FST (~4ms when indexed; falls back to live-scan). Prefer over Read+manual scanning when you want to know what a function calls without reading the whole body — callees gives the resolved outgoing edges as records. Phase 14.2 + 14.2.2: Python/Java decorators, Kotlin annotations, and C# method/constructor attributes are surfaced as callees of the decorated function (decorator factories like `@lru_cache(maxsize=128)` or `@Inject` appear as the factory/annotation name `lru_cache` / `Inject`, alongside regular body calls).",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "symbol": { "type": "string", "description": "Exact function name — canonical key (v1.7+)." },
