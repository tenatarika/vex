...
## 10. No cross-language edges; `--exclude-generated` is a header heuristic

### 10.1 vex does not link symbols across languages

`vex usages` / `callers` / `impact` stop at the language boundary. A TypeScript
`fetch("/api/v1/invoices")` and the Go handler registered for that route share
no symbol, so no edge exists to follow, and vex does not synthesise one.

This is a deliberate scope decision, not an oversight. In the benchmark
accompanying a multivocal literature review of microservice architecture-recovery
tools ([arXiv:2412.08352](https://arxiv.org/abs/2412.08352), later in *Empirical
Software Engineering*), REST-endpoint extraction topped out at **0.68 recall**
(RAD), and **three of the six tools that attempted it recovered zero endpoints**;
the best source-level performer (RAD-source) is Java/Spring-only and reached 0.54.
The review's own conclusion is that the high-precision results come from parsing
deployment manifests rather than source.

The tools that do cross the boundary well require a **declared interface
artifact** as the join key rather than matching strings across languages:
JetBrains' Endpoints window mediates JavaScript↔backend through an OpenAPI spec
and supports only `fetch`/axios on the client side
([docs](https://www.jetbrains.com/help/idea/endpoints-tool-window.html)), and
Meta's Glean anchors its cross-language RPC navigation on Thrift/protobuf IDL
([engineering.fb.com](https://engineering.fb.com/2024/12/19/developer-tools/glean-open-source-code-indexing/)).

An edge that is silently wrong a third of the time is worse than no edge for an
agent, which cannot tell which third.

**What to do instead**: search for the string both sides share — the route path,
queue topic, env-var key, or proto message name — and pivot back to structure
once you have a name. `vex grep` is backed by a per-file trigram skip-index, so
a full-repo literal scan is cheap. Worked example: `docs/COOKBOOK.md` → Recipe 6.

**What this cannot see**, and no static tool can: dynamically constructed URLs
(string concatenation, base-URL variables), gateway/service-mesh rewrites, and
config-driven routing. For "which services call this **in production**",
distributed tracing is the right instrument; source search is complementary
because it also sees cold paths and un-exercised routes that tracing never
observes.

### 10.2 `--exclude-generated` under-reports by design

`vex search --exclude-generated` recognises machine-generated files from their
**header banner** (`// Code generated … DO NOT EDIT.`, protoc's
`Generated by the protocol buffer compiler.  DO NOT EDIT!`, sqlc, rust-bindgen,
Diesel CLI, OpenAPI Generator, Thrift, javah, and the `@generated` annotation).

There is no cross-ecosystem convention: GitHub Linguist, the most complete
public catalogue of this problem, dispatches to roughly seventy separate
per-generator predicates
([`lib/linguist/generated.rb`](https://github.com/github-linguist/linguist/blob/main/lib/linguist/generated.rb)),
each with its own marker string and line window. So the checks here are a table
too, and the table is not exhaustive. Consequences:

- **Generators that write no banner are invisible.** `cbindgen` emits nothing by
  default; `rust-bindgen` records its own version but never the input header.
- **Provenance is usually absent.** Only the protoc family and sqlc record a
  `// source: <path>` line. Where present, that path is relative to the
  generator's import root, **not** to the repository, so it must be
  suffix-matched rather than compared for equality.
- **Build-time generation leaves nothing to detect.** If stubs are generated
  during the build rather than checked in, they are not in the index at all.
- The filter therefore **fails towards keeping results**: an unreadable file, a
  missing banner, or non-UTF-8 content all count as "not generated". It may
  leave generated files in the output; it will not hide a file that never
  carried a generator banner.
- **The converse is not guaranteed.** A generated file that someone later
  hand-edited still carries its banner and is indistinguishable from a pristine
  one, so it *will* be filtered. Detection is banner-based, not
  provenance-based. If you patch vendored generated code in place, expect
  `--exclude-generated` to hide your edits.

It applies to `vex search` only, and post-filters results by reading each
candidate file's first 8 KB (memoised per path, evaluated lazily until the
result limit is reached) — unlike every other filter, which answers from the
index alone.
