...
This is a deliberate scope decision, not an oversight. Published static
extractors for exactly this problem (REST endpoint + client recovery across a
microservice fleet) top out around **0.68 recall**, and in one benchmark of six
tools **three recovered zero endpoints**; the best source-level performer is
single-language, single-framework. The tools that do cross the boundary well
(JetBrains Endpoints, Meta's Glean) require a **declared interface artifact** —
OpenAPI, protobuf, Thrift — as the join key, and do not match strings across
languages directly. An edge that is silently wrong a third of the time is worse
than no edge for an agent, which cannot tell which third.
...
There is no cross-ecosystem convention — GitHub Linguist carries ~71 separate
predicates for this, one per generator — so the checks are a table, and the
table is not exhaustive. Consequences:
...
- The filter therefore **fails towards keeping results**: an unreadable file, a
  ...
  leave generated files in the output; it should not hide hand-written code.
