...
- **Persistent call graph** — `vex callers`/`vex callees` reads from an FST built at index time (~4ms), not a live tree-sitter scan (seconds). Module-scope expressions are reported via synthetic `<module:path>` callers (Phase 14.1); Python and Java function/method decorators emit forward edges to their targets (Phase 14.2). TS/Rust/Kotlin/C# decorators and class-level decorators remain invisible — see [`docs/LIMITATIONS.md`](docs/LIMITATIONS.md)
...
- **`vex callers` has uneven coverage outside function scope.** Module-level expressions like `app = create_app()` are reported via synthetic `<module:path>` callers (Phase 14.1). Python and Java function/method decorators emit forward edges (Phase 14.2) — `vex callers GetMapping` lists every Spring handler. TypeScript / Rust decorators (14.2.1), Kotlin / C# decorators (14.2.2), and class-level decorators (14.6) remain on the roadmap.
...
- **`vex callers` outside function scope** — Module-level expressions
  ...
  Python and Java function/method decorators emit forward edges
  (Phase 14.2). Remaining gaps: TypeScript / Rust decorators (14.2.1),
  Kotlin / C# decorators (14.2.2), and class-level decorators (14.6).
