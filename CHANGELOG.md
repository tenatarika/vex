...
- **`crate::util::ident::is_identifier_shaped`.** Promoted from `cli::cmd_search` (was a private fn used only by the v1.15.0 search-drift hint) so the new `HistoryReader::find_by_name_or_prefix` can reuse the same conservative definition. Test module in `cmd_search` retargets the `use` to `crate::util::ident::*`.
